  ...
  <div class="container-fluid">
    ...
    <div class="flex-row navbar-nav d-lg-none">
      ...
      <div class="d-none d-lg-flex">
        <a href="?theme=dark" class="px-0 nav-link hide-theme-dark" data-bs-toggle="tooltip" data-bs-placement="bottom" aria-label="Enable dark mode" data-bs-original-title="Enable dark mode">
        ...
        <a href="?theme=light" class="px-0 nav-link hide-theme-light" data-bs-toggle="tooltip" data-bs-placement="bottom" aria-label="Enable light mode" data-bs-original-title="Enable light mode">
    ...
    <div class="collapse navbar-collapse" id="sidebar-menu">
      <ul class="navbar-nav pt-lg-3">
        ...
        @php
        $sidebarController = new \App\Http\Controllers\Admin\SidebarController();
        $sidebarItems = $sidebarController->index();
        $menuItems = $sidebarItems->groupBy('parent_id');
        @endphp
        
        @if(isset($menuItems[0]) && $menuItems[0]->isNotEmpty())
            @foreach($menuItems[0] as $parentItem)
                @if(auth()->user()->is_superadmin || auth()->user()->hasPermission($parentItem->permission->title))
                    @if(isset($menuItems[$parentItem->id]))
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle" href="javascript:void(0);" data-bs-toggle="dropdown" aria-expanded="false">
                                <span class="nav-link-icon d-md-none d-lg-inline-block">
                                  <i class="ti {{ $parentItem->icon }}" style="font-size: 20px;"></i>
                                </span>
                                <span class="nav-link-title">
                                    {{ $parentItem->menu_item }}
                                </span>
                            </a>
                            <div class="dropdown-menu">
                                @foreach($menuItems[$parentItem->id]->sortBy('order') as $subItem)
                                    @if(auth()->user()->is_superadmin || auth()->user()->hasPermission($subItem->permission->title))
                                        <a class="dropdown-item {{ request()->is($subItem->url) ? 'active' : '' }}" href="{{ $subItem->url }}">
                                            {{ $subItem->menu_item }}
                                        </a>
                                    @endif
                                @endforeach
                    ...
                    @else
                        <li class="nav-item">
                            <a class="nav-link {{ request()->is($parentItem->url) ? 'active' : '' }}" href="{{ $parentItem->url }}">
                                <span class="nav-link-icon d-md-none d-lg-inline-block">
                                  <i class="ti {{ $parentItem->icon }}" style="font-size: 20px;"></i>
                                </span>
                                <span class="nav-link-title">
        ...
        @else
            <li class="nav-item">
                <div class="nav-link">
                    <div>No routes are available</div>
                </div>
            </li>
        @endif
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                ...
                menuItems.forEach(item => {
                    const menuLink = item.querySelector('a.nav-link');
                    if (menuLink && menuLink.getAttribute('href') === currentUrl) {
                        item.classList.add('active');
                        foundActive = true;
        
                        // Check for parent item and set it as open if it's a sub-menu
                        const parentItem = item.closest('.dropdown');
                        if (parentItem) {
                ...
                // If no active item was found, check sessionStorage for the activeMenuId
                if (!foundActive) {
                    const activeMenuId = sessionStorage.getItem('activeMenuId');
                    if (activeMenuId) {
                        const activeItem = document.querySelector(`.nav-item[data-menu-id="${activeMenuId}"]`);
                        if (activeItem) {
                            activeItem.classList.add('active');
                            const parentItem = activeItem.closest('.dropdown');
                            if (parentItem) {
                                parentItem.classList.add('show');
                            }
                        }
                ...
                menuItems.forEach(item => {
                    item.addEventListener('click', function() {
                        ...
                        // Store the active menu ID in session storage
